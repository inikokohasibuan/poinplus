...
@section('page')
<h1 class="mb-4">{{ isset($subproduk) ? 'Copy Sub Produk' : 'Tambah Sub Produk' }}</h1>
@stop

@section('content')
<div class="card">
    <div class="card-body">
        <form action="{{ route('master.subproduk.store') }}" method="POST">
            @csrf
            <div class="form-group">
                ...
                <select name="id_produk" id="id_produk" class="form-control" required>
                    ...
                    @foreach($produk as $item)
                        <option value="{{ $item->id_produk }}" {{ isset($subproduk) && $subproduk->id_produk == $item->id_produk ? 'selected' : '' }}>{{ $item->nama_produk }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <button type="submit" class="btn btn-primary">{{ isset($subproduk) ? 'Copy' : 'Simpan' }}</button>
        </form>
    </div>
</div>
@stop
